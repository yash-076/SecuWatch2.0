...
from config import DEVICE_ID, API_KEY, WATCH_FILES
from core import (
    agent_state,
...
def file_monitoring_loop(stop_event: threading.Event) -> None:
    # Filter watch files to only unique strings
    unique_paths = list(set(WATCH_FILES))
    tailers = [FileTailer(p) for p in unique_paths]
    print(f"[TAILER] Monitoring {len(tailers)} file paths: {unique_paths}")
        ...
        while not stop_event.is_set():
            for tailer in tailers:
                ...
                for line in lines:
                    ...
                    if any(kw in line_lower for kw in ["error", "critical", "exception", "failed"]):
                        level = "ERROR"
                    ...
                    print(f"Sent log (File) | message=\"{enriched[:120]}...\"")
                        ...
                        if threat_info:
                            ...
                            if is_severity_met(severity, threshold):
                                ...
                                alert_payload = {
                                    ...
                                    "type": alert_type,
                                    "severity": severity,
                                    "description": f"[LOCAL DETECT] Local agent threat signature '{alert_type}' matches file log: {line}",
                                }
...
def main() -> None:
    ...
    print("[WEB-LOG-AGENT] Starting SecuWatch Web/Apache Log Tailer...")
    if not WATCH_FILES:
        print("[WEB-LOG-AGENT] [WARNING] No files configured to watch. Configure them in agent_config.json")
        
    heartbeat_thread = start_heartbeat_thread(stop_event)
    
    file_thread = threading.Thread(target=file_monitoring_loop, args=(stop_event,), daemon=True)
    file_thread.start()
    ...
    try:
        ...
        print("Stopping web log agent...")
        stop_event.set()
        heartbeat_thread.join(timeout=2)
        file_thread.join(timeout=2)
        print("Web log agent stopped.")
